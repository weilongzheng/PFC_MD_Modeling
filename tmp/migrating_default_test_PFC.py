...
for task_pair_id in range(len(task_pairs)):
    ...
    # training
    total_training_cycle = 50000//config['batch_size']
    print_every_cycle = 1000//config['batch_size']
    running_loss = 0.0
    ...
    training_cycles_bar = trange(total_training_cycle)
    for i in range(total_training_cycle):
        ...
        binputs, blabels = get_trials_batch(env, config['batch_size'])
        inputs, labels = binputs.permute([1,0,2]), blabels.permute([1,0,2])
    ...
    ax = axes[0,0]
    env_id = 0
    ax.set_title(task_pair[env_id])
    ax.plot( log['losses'])
    ax = axes[0,1]
    ax.plot(log['stamps'],log['act_perfs'][env_id])
    ax = axes[1,0]
    ax.plot(log['stamps'],log['fix_perfs'][env_id])
    mdbool = config['MDeffect']
    fig.savefig('./files/migrating/'+f'{task_pair[env_id][7:]}_log_MD_{mdbool}'+'.jpg')
