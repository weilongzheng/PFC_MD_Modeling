...
# SI
if config['SI']:
    args = Args(lr=config['lr'], c=config['SI_c'], xi=config['SI_xi'], batch_size=config['batch_size'])
    si = SI(backbone=net, loss=criterion, args=args, transform=None, opt=optimizer, device=device)
    net = si.net
...
for i in range(total_training_cycle):
    ...
    if config['SI']:
        loss, rnn_activity = si.observe(inputs, labels, not_aug_inputs=None, sub_id=task_id)
