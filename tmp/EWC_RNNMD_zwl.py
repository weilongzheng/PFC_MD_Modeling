...
from model_ewc_zwl import ElasticWeightConsolidation
...
if config['EWC']: 
    ...
    # EWC
    ewc = ElasticWeightConsolidation(net, crit=criterion, parameters = training_params, named_parameters = named_training_params, lr=config['lr'], weight=config['EWC_weight'])
    ...
    for i in range(total_training_cycle):
        ...
        ewc.forward_backward_update(inputs, labels, sub_id=task_id)
        outputs, rnn_activity = ewc.model(inputs, sub_id=task_id)
        ...
        if i % print_every_cycle == (print_every_cycle - 1):
            ...
            with torch.no_grad():
                ...
                for env_id in range(len(tasks)):
                    fix_perf, act_perf = get_full_performance(ewc.model, test_envs[env_id], task_id=env_id, num_task=len(tasks), num_trial=100, device=device) # set large enough num_trial to get good statistics
    ...
    np.save('./files/'+'log_noMD.npy', log)
###--------------------------Analysis--------------------------###
