    ...
    def init_weights(self):
        ...
        for i in range(self.wMD2PFC.shape[0]):
            if np.random.rand() < self.config.MDtoPFC_connect_prob:
                ...
                self.wMD2PFC[i, j] = 2.5 # +5, dis-inhibition version # -5, invert version
...
class CTRNN_MD(nn.Module):
    ...
    def recurrence(self, input, sub_id, hidden):
        ...
        if self.MDeffect:
            ...
            md2pfc = np.dot((self.md.wMD2PFC/self.md.num_active), self.md.md_output) - 2.5/self.md.num_active # - 5/self.md.md_size, the base level of inhibition; check the initialization of wMD2PFC to understand "5" # md2pfc = np.dot((self.md.wMD2PFC/self.md.md_size), np.logical_not(self.md.md_output).astype(float)) # invert version
            md2pfc = torch.from_numpy(md2pfc).view_as(hidden).to(input.device)
