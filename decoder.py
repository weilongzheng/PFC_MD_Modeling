...
if __name__ == '__main__':
    ...
    #num_MD = [8,16]
    tsteps = 200
    acc_rule_pfc_all = np.zeros([len(Tau_times),tsteps])
    acc_rule_md_all = np.zeros([len(Tau_times),tsteps])
    acc_context_pfc_all = np.zeros([len(Tau_times),tsteps])
    acc_context_md_all = np.zeros([len(Tau_times),tsteps])
    
    for i,itau in enumerate(Tau_times):
        pickle_in = open('dataPFCMD/test'+\
                         '_MDTrue_LearnTrue_R8_'+\
        ...
        acc_rule_md = list()
        for i_time in range(tsteps):
            X, y = sklearn.utils.shuffle(MDinps_all[:,i_time,:],rule_label)
            X_train, X_test = X[:n_train],X[n_train:]
            y_train, y_test = y[:n_train],y[n_train:]
        ...
        acc_context_md = list()
        for i_time in range(tsteps):
            X, y = sklearn.utils.shuffle(MDinps_all[:,i_time,:],context_label)
            X_train, X_test = X[:n_train],X[n_train:]
            y_train, y_test = y[:n_train],y[n_train:]
    ...
    pickle_out = open('decoding_results_MDlearn_TauSeed8.pickle','wb')
