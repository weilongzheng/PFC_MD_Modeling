...
def plotRout():
    ...
    c=np.mean(routs_mean[cue1,:],axis=1)
    plt.figure(figsize=(2.4,2.4))
    plt.plot(c.T,color='tab:orange',linewidth=0.5)
    plt.xlabel('Neuron #')
    plt.ylabel('Mean Activity (a.u.)')
...
def plotPFCnorm():
    ...
    data_plot = np.array([np.mean(routs_mean_nomd,axis=0),np.mean(routs_mean_md,axis=0)])
    #sns.distplot(routs_mean_nomd.flatten(), hist=True, kde=True, color = 'tab:red', kde_kws = {'linewidth': 2});
    sns.histplot(data=data_plot.T, stat = 'probability', bins = bins, kde=True)
