# -*- coding: utf-8 -*-
"""
Created on Fri Jan 29 14:13:30 2021
...
if __name__ == '__main__':
    ...
    tsteps = 2#200
    acc_rule_pfc_all = np.zeros([len(config_par_range),round(tsteps/2)])
    acc_rule_md_all = np.zeros([len(config_par_range),round(tsteps/2)])
    acc_context_pfc_all = np.zeros([len(config_par_range),round(tsteps/2)])
    acc_context_md_all = np.zeros([len(config_par_range),round(tsteps/2)])
    
    for i,itau in enumerate(config_par_range):
        pickle_in = open('dataPFCMD/testConfig_HebbPostTrace_numMD'+str(12)+'_numTask'+str(2)+'_MD'+\
                                    str(True)+\
                                    '_Learn'+str(True)+\
                                    '_R'+str(5)+\
                                    '_TimesTau'+str(4)+\
                                    '_inpsPerTask'+str(itau)+\
                                    '_Trace10'+\
                                    '.pickle','rb')
...
plt.subplot(1,4,1,autoscale_on=True)
plt.plot(acc_rule_pfc_all,'-rv',label='PFC')
plt.legend(frameon=False)
plt.xlabel('Num Cues per Context') #plt.xlabel('$\\tau_{MD}/\\tau_{PFC}$')
plt.xticks(np.arange(5),config_par_range)
plt.ylabel('Decoding Rule')
plt.ylim([0.45, 1.05])
plt.subplot(1,4,2,autoscale_on=True)
plt.plot(acc_rule_md_all,'-bv',label='MD')
plt.legend(frameon=False)
plt.xlabel('Num Cues per Context') #plt.xlabel('$\\tau_{MD}/\\tau_{PFC}$')
plt.xticks(np.arange(5),config_par_range)
plt.ylabel('Decoding Rule')
plt.ylim([0.45, 1.05])
plt.subplot(1,4,3,autoscale_on=True)
plt.plot(acc_context_pfc_all,'-rv',label='PFC')
plt.legend(frameon=False)
plt.xlabel('Num Cues per Context') #plt.xlabel('$\\tau_{MD}/\\tau_{PFC}$')
plt.xticks(np.arange(5),config_par_range)
plt.ylabel('Decoding Context')
plt.ylim([0.45, 1.05])
plt.subplot(1,4,4,autoscale_on=True)
plt.plot(acc_context_md_all,'-bv',label='MD')
plt.legend(frameon=False)
plt.xlabel('Num Cues per Context') #plt.xlabel('$\\tau_{MD}/\\tau_{PFC}$')
plt.xticks(np.arange(5),config_par_range)
plt.ylabel('Decoding Context')
